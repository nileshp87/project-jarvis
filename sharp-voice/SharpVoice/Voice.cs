    ...
    {   
		...
		*/
        ...
        public Folder SMS
        {
            get
            {
                if (_sms == null || _sms.NeedsUpdate)
                {
                    ...
                    XmlDocument xd = new XmlDocument();
                    xd.LoadXml(box);
                    XmlNode root = xd.SelectSingleNode("//response/html");
                    string json = xd.SelectSingleNode("//response/json").InnerText;
        ...
        public Message mostRecent
        {
            get
            {
                HttpWebRequest request = (HttpWebRequest)WebRequest.Create(dict["XML_SMS"]);
                ...
                {
                    var rssReader = XmlTextReader.Create(response.GetResponseStream());
                    string s;
                    while (rssReader.Read())
                    {
                        s = rssReader.ReadInnerXml();
                        if (s.Contains("gc-message-sms-row"))
                        {
                            string m = s.Substring(s.LastIndexOf("gc-message-sms-row"), 400);
                            if (m.Substring(74, 3) == "Me:")
                            {
                                s = s.Substring(0, s.LastIndexOf("gc-message-sms-row"));
                                return re;
                            }
                            else
                            {
                                re.displayNumber = m.Substring(m.IndexOf("<span class=\"gc-message-sms-from\">") + 39, m.IndexOf("</span>") - m.IndexOf("<span class=\"gc-message-sms-from\">") - 43);
                                re.Text = m.Substring(m.IndexOf("<span class=\"gc-message-sms-text\">") + 34, m.IndexOf("</span>", m.IndexOf("<span class=\"gc-message-sms-text\">")) - m.IndexOf("<span class=\"gc-message-sms-text\">") - 34);
                                re.ID = re.displayNumber + re.Text;
                            }
                        }
                    }
                    return re;
